...
matches = bf.match(descriptors3, descriptors2)
...
src_points = np.float32([keypoints3[match.queryIdx].pt for match in matches]).reshape(-1, 1, 2)
...
result = cv2.warpPerspective(image3, homography, (size_x, size_y))


# Blending the warped image with the second image using alpha blending
alpha = 0.5  # blending factor
blended_image_p1 = cv2.addWeighted(result, alpha, image2, 1 - alpha, 0)

# Display the blended image
cv2.imwrite(project_folder + '2.merged_M-R.jpeg', blended_image_p1)
